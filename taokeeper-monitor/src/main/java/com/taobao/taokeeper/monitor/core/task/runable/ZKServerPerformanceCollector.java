				...
				if ( 0 < difference ) {
					...
					if ( GlobalInstance.needAlarm.get() ) {
						ThreadPoolManager.addJobToMessageSendExecutor( new TbMessageSender( 
								new Message( emailList, "ZK,Server cpu usage too high-" + clusterName, hostPerformanceEntity.getIp() + " cpu usage too high! " + cpuUsage + "-" + maxCpuUsage + "=" + difference,
				...
				if ( 0 < difference ) {
					...
					if ( GlobalInstance.needAlarm.get() ) {
						ThreadPoolManager.addJobToMessageSendExecutor( new TbMessageSender( 
								...
								new Message( emailList, "ZK Server memory usage too high:-" + clusterName, hostPerformanceEntity.getIp() + " memory too high：" + memoryUsage + "-" + maxMemoryUsage + "=" + difference,
								Message.MessageType.EMAIL ),
				...
				if ( 0 < difference ) {
					...
					if ( GlobalInstance.needAlarm.get() ) {
						...
						ThreadPoolManager.addJobToMessageSendExecutor( new TbMessageSender( 
								...
								new Message( emailList, " ZK Server load usage too high-"
