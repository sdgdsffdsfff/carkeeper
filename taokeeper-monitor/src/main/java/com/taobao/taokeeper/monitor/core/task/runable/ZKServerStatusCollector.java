...
public class ZKServerStatusCollector implements Runnable {
    ...
    private void sshZooKeeperAndHandleStat(String ip, int port, ZooKeeperStatus zooKeeperStatus) {
        ...
        try {
            System.out.println("mylog: ip=" + ip + "\nport=" + SystemConstant.portOfSSH + "\nuserNameOfSSH=" + userNameOfSSH + "\npasswordOfSSH=" + passwordOfSSH + "\ncommand=" + StringUtil.replaceSequenced(COMMAND_STAT, ip, port + EMPTY_STRING));
        ...
        try {
            ...
            if (needAlarm) {
                ...
                if (GlobalInstance.needAlarm.get()) {
                    String emailList = alarmSettings.getEmailList();
                    String phoneList = alarmSettings.getPhoneList();

                    ThreadPoolManager.addJobToMessageSendExecutor(new TbMessageSender(new Message(emailList, "ZooKeeper连接数，Watcher数报警-" + clusterName, clusterName + "-" + sb.toString(), Message.MessageType.EMAIL)));
                    ThreadPoolManager.addJobToMessageSendExecutor(new TbMessageSender(new Message(phoneList, "ZooKeeper连接数，Watcher数报警-" + clusterName, clusterName + "-" + sb.toString(), Message.MessageType.SMS)));
                }
            }// need alarm
    ...
    private void storeTaoKeeperStatToDB(int clusterId, ZooKeeperStatus zooKeeperStatus) {
        ...
        try {
            ...
            if (zooKeeperStatus.getSent() == null || zooKeeperStatus.getReceived() == null) {
                return;
            }
            reportDAO.addTaoKeeperStat(new TaoKeeperStat(clusterId,
                    ...
                    Long.parseLong(zooKeeperStatus.getSent() == null ? "0" : zooKeeperStatus.getSent()),
                    Long.parseLong(zooKeeperStatus.getReceived() == null ? "0" : zooKeeperStatus.getReceived()),
    ...
    public static void main(String[] args) {
        System.out.println(StringUtil.replaceSequenced(COMMAND_STAT, "127.0.0.1", "2181" + EMPTY_STRING));
    }
